...
import edu.pnu.stem.dao.publicsafety.PublicSafetyTransformerDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyTransformer;

@RestController
@RequestMapping("/documents/{docId}/publicsafetytransformer")
public class PublicSafetyTransformerController {
	...
	public void createPublicSafetyTransformer(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			s = PublicSafetyTransformerDAO.createPublicSafetyTransformer(map, parentId, id, name, description, geometry, duality, connected);
	...
	public void updatePublicSafetyTransformer(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			PublicSafetyTransformerDAO.updatePublicSafetyTransformer(map, parentId, id, name, description,	geom, duality, connects);
	...
	public void getPublicSafetyTransformer(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyTransformerDAO.readPublicSafetyTransformer(map, id));
	...
	public void deletePublicSafetyTransformer(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			PublicSafetyTransformerDAO.deletePublicSafetyTransformer(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
