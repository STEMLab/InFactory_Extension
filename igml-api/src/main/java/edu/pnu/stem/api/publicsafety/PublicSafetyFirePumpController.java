...
import edu.pnu.stem.dao.publicsafety.PublicSafetyFirePumpDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyFirePump;

@RestController
@RequestMapping("/documents/{docId}/publicsafetyfirePump")
public class PublicSafetyFirePumpController {
	...
	public void createPublicSafetyFirePump(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			s = PublicSafetyFirePumpDAO.createPublicSafetyFirePump(map, parentId, id, name, description, geometry, duality, connected);
	...
	public void updatePublicSafetyFirePump(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			PublicSafetyFirePumpDAO.updatePublicSafetyFirePump(map, parentId, id, name, description,	geom, duality, connects);
	...
	public void getPublicSafetyFirePump(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyFirePumpDAO.readPublicSafetyFirePump(map, id));
	...
	public void deletePublicSafetyFirePump(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			PublicSafetyFirePumpDAO.deletePublicSafetyFirePump(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
