...
import edu.pnu.stem.dao.publicsafety.PublicSafetyElevatorDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyElevator;

@RestController
@RequestMapping("/documents/{docId}/publicsafetyelevator")
public class PublicSafetyElevatorController {
	@Autowired
    private ApplicationContext applicationContext;
	
	@PostMapping(value = "/{id}", produces = "application/json")
	@ResponseStatus(HttpStatus.CREATED)
	public void createPublicSafetyElevator(@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			 ...
			 * */
			c = PublicSafetyElevatorDAO.createPublicSafetyElevator(map, parentId, id, name, description, generalGeometry, duality, partialBoundedBy, classType, functionType, usageType);
	...
	public void getPublicSafetyElevator(@PathVariable("docId") String docId,@PathVariable("id") String id, HttpServletRequest request, HttpServletResponse response) throws IOException {
		
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			
			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyElevatorDAO.readPublicSafetyElevator(map, id));
	...
	public void updatePublicSafetyElevator(@PathVariable("docId") String docId,@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			String duality = null;
		...
		PublicSafetyElevatorDAO.updatePublicSafetyElevator(map, parentId, id, null, null, geom, duality, partialBoundedBy );
	...
	public void deletePublicSafetyElevator(@PathVariable("docId") String docId,@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);			
			PublicSafetyElevatorDAO.deletePublicSafetyElevator(map, id);
		}
		catch(NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
