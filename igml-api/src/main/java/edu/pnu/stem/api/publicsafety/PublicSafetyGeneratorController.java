...
import edu.pnu.stem.dao.publicsafety.PublicSafetyGeneratorDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyGenerator;

@RestController
@RequestMapping("/documents/{docId}/publicsafetygenerator")
public class PublicSafetyGeneratorController {
	...
	public void createPublicSafetyGenerator(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			s = PublicSafetyGeneratorDAO.createPublicSafetyGenerator(map, parentId, id, name, description, geometry, duality, connected);
	...
	public void updatePublicSafetyGenerator(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			PublicSafetyGeneratorDAO.updatePublicSafetyGenerator(map, parentId, id, name, description,	geom, duality, connects);
	...
	public void getPublicSafetyGenerator(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyGeneratorDAO.readPublicSafetyGenerator(map, id));
	...
	public void deletePublicSafetyGenerator(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			PublicSafetyGeneratorDAO.deletePublicSafetyGenerator(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
