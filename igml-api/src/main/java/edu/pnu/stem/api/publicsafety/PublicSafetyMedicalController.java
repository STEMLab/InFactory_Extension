...
import edu.pnu.stem.dao.publicsafety.PublicSafetyMedicalDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyMedical;

@RestController
@RequestMapping("/documents/{docId}/publicsafetymedical")
public class PublicSafetyMedicalController {
	...
	public void createPublicSafetyMedical(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			s = PublicSafetyMedicalDAO.createPublicSafetyMedical(map, parentId, id, name, description, geometry, duality, connected);
	...
	public void updatePublicSafetyMedical(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			PublicSafetyMedicalDAO.updatePublicSafetyMedical(map, parentId, id, name, description,	geom, duality, connects);
	...
	public void getPublicSafetyMedical(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyMedicalDAO.readPublicSafetyMedical(map, id));
	...
	public void deletePublicSafetyMedical(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			PublicSafetyMedicalDAO.deletePublicSafetyMedical(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
