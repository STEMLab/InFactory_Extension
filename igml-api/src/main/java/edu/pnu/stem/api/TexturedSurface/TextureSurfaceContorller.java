package edu.pnu.stem.api.TexturedSurface;
...
import edu.pnu.stem.dao.TextruedSurface.TextureSurfaceDAO;
import edu.pnu.stem.dao.core.CellSpaceBoundaryDAO;
import edu.pnu.stem.feature.texture.TextureSurface;

@RestController
@RequestMapping("documents/{docId}/texturesurface")
public class TextureSurfaceContorller {
	...
	public void createTexturedSurface(@PathVariable("docId") String docId,@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		...
		if(json.has("TextureCoordinate")) {
			textureCoodi = Convert2Json.json2Geometry(json.get("TextureCoordinate"));
			
		}
		if(json.has("TextureImage")) {
			textureImage = json.get("TextureImage").asText().trim();
			
		}
		TextureSurface c = null;
		try {
			 ...
			 * */			
			c = TextureSurfaceDAO.createTextureSurface(map, parentId, id, name, description, geometry, duality, textureCoodi, textureImage); 
