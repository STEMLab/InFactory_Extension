...
import edu.pnu.stem.dao.publicsafety.PublicSafetySprinklerDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetySprinkler;

@RestController
@RequestMapping("/documents/{docId}/publicsafetysprinkler")
public class PublicSafetySprinklerController {
	...
	public void createPublicSafetySprinkler(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			s = PublicSafetySprinklerDAO.createPublicSafetySprinkler(map, parentId, id, name, description, geometry, duality, connected);
	...
	public void updatePublicSafetySprinkler(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			PublicSafetySprinklerDAO.updatePublicSafetySprinkler(map, parentId, id, name, description,	geom, duality, connects);
	...
	public void getPublicSafetySprinkler(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetySprinklerDAO.readPublicSafetySprinkler(map, id));
	...
	public void deletePublicSafetySprinkler(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			PublicSafetySprinklerDAO.deletePublicSafetySprinkler(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
