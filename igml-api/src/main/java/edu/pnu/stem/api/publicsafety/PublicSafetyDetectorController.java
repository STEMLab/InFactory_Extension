...
import edu.pnu.stem.dao.publicsafety.PublicSafetyDetectorDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyDetector;

@RestController
@RequestMapping("/documents/{docId}/PublicSafetyDetector")
public class PublicSafetyDetectorController {
	...
	public void createPublicSafetyDetector(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			s = PublicSafetyDetectorDAO.createPublicSafetyDetector(map, parentId, id, name, description, geometry, duality, connected);
	...
	public void updatePublicSafetyDetector(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			PublicSafetyDetectorDAO.updatePublicSafetyDetector(map, parentId, id, name, description,	geom, duality, connects);
	...
	public void getPublicSafetyDetector(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyDetectorDAO.readPublicSafetyDetector(map, id));
	...
	public void deletePublicSafetyDetector(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			PublicSafetyDetectorDAO.deletePublicSafetyDetector(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
