...
import edu.pnu.stem.dao.publicsafety.PublicSafetyEscalatorDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyEscalator;

@RestController
@RequestMapping("/documents/{docId}/publicsafetyescalator")
public class PublicSafetyEscalatorController {
	...
	public void createPublicSafetyEscalator(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			s = PublicSafetyEscalatorDAO.createPublicSafetyEscalator(map, parentId, id, name, description, geometry, duality, connected);
	...
	public void updatePublicSafetyEscalator(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			PublicSafetyEscalatorDAO.updatePublicSafetyEscalator(map, parentId, id, name, description,	geom, duality, connects);
	...
	public void getPublicSafetyEscalator(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyEscalatorDAO.readPublicSafetyEscalator(map, id));
	...
	public void deletePublicSafetyEscalator(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			PublicSafetyEscalatorDAO.deletePublicSafetyEscalator(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
