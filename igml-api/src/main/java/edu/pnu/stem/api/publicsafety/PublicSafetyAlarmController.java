...
import edu.pnu.stem.dao.publicsafety.PublicSafetyAlarmDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyAlarm;

@RestController
@RequestMapping("/documents/{docId}/publicsafetyalarm")
public class PublicSafetyAlarmController {
	...
	public void createPublicSafetyAlarm(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			s = PublicSafetyAlarmDAO.createPublicSafetyAlarm(map, parentId, id, name, description, geometry, duality, connected);
	...
	public void updatePublicSafetyAlarm(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			PublicSafetyAlarmDAO.updatePublicSafetyAlarm(map, parentId, id, name, description,	geom, duality, connects);
	...
	public void getPublicSafetyAlarm(@PathVariable("docId") String docId, @PathVariable("id") String id,
			HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);

			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyAlarmDAO.readPublicSafetyAlarm(map, id));
	...
	public void deletePublicSafetyAlarm(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			PublicSafetyAlarmDAO.deletePublicSafetyAlarm(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
