...
import edu.pnu.stem.dao.publicsafety.PublicSafetyStairDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyStair;

@RestController
@RequestMapping("/documents/{docId}/publicsafetystair")
public class PublicSafetyStairController {
	@Autowired
    private ApplicationContext applicationContext;
	
	@PostMapping(value = "/{id}", produces = "application/json")
	@ResponseStatus(HttpStatus.CREATED)
	public void createPublicSafetyStair(@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			 ...
			 * */
			c = PublicSafetyStairDAO.createPublicSafetyStair(map, parentId, id, name, description, generalGeometry, duality, partialBoundedBy, classType, functionType, usageType);
	...
	public void getPublicSafetyStair(@PathVariable("docId") String docId,@PathVariable("id") String id, HttpServletRequest request, HttpServletResponse response) throws IOException {
		
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			
			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyStairDAO.readPublicSafetyStair(map, id));
	...
	public void updatePublicSafetyStair(@PathVariable("docId") String docId,@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			String duality = null;
		...
		PublicSafetyStairDAO.updatePublicSafetyStair(map, parentId, id, null, null, geom, duality, partialBoundedBy );
	...
	public void deletePublicSafetyStair(@PathVariable("docId") String docId,@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);			
			PublicSafetyStairDAO.deletePublicSafetyStair(map, id);
		}
		catch(NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
