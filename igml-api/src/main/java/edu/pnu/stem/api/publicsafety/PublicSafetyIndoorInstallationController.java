...
import edu.pnu.stem.dao.publicsafety.PublicSafetyIndoorInstallationDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyIndoorInstallation;

@RestController
@RequestMapping("/documents/{docId}/publicsafetyindoorinstallation")
public class PublicSafetyIndoorInstallationController {
	...
	public void createPublicSafetyIndoorInstallation(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		PublicSafetyIndoorInstallation s;
		...
		try {
			...
			s = PublicSafetyIndoorInstallationDAO.createPublicSafetyIndoorInstallation(map, parentId, id, name,
					description, geometry, duality, connected);
	...
	public void updatePublicSafetyIndoorInstallation(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			...
			PublicSafetyIndoorInstallationDAO.updatePublicSafetyIndoorInstallation(map, parentId, id, name, description,
					geom, duality, connects);
	...
	public void getPublicSafetyIndoorInstallation(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			ObjectNode target = Convert2Json.convert2JSON(map,
					PublicSafetyIndoorInstallationDAO.readPublicSafetyIndoorInstallation(map, id));
	...
	public void deletePublicSafetyIndoorInstallation(@PathVariable("docId") String docId, @PathVariable("id") String id,
			@RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			PublicSafetyIndoorInstallationDAO.deletePublicSafetyIndoorInstallation(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
