...
import edu.pnu.stem.dao.publicsafety.PublicSafetyRoomDAO;
import edu.pnu.stem.feature.publicsafety.PublicSafetyRoom;

@RestController
@RequestMapping("/documents/{docId}/publicsafetyroom")
public class PublicSafetyRoomController {
	@Autowired
    private ApplicationContext applicationContext;
	
	@PostMapping(value = "/{id}", produces = "application/json")
	@ResponseStatus(HttpStatus.CREATED)
	public void createPublicSafetyRoom(@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(json.has("properties")){
			...
			if(json.get("properties").has("psRoomType")) {
				psRoomType = json.get("properties").get("psRoomType").asText().trim();
		...
		try {
			 ...
			 * */
			c = PublicSafetyRoomDAO.createPublicSafetyRoom(map, parentId, id, name, description, generalGeometry, duality, partialBoundedBy, classType, functionType, usageType, psRoomType);
	...
	public void getPublicSafetyRoom(@PathVariable("docId") String docId,@PathVariable("id") String id, HttpServletRequest request, HttpServletResponse response) throws IOException {
		
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			
			ObjectNode target = Convert2Json.convert2JSON(map, PublicSafetyRoomDAO.readPublicSafetyRoom(map, id));
	...
	public void updatePublicSafetyRoom(@PathVariable("docId") String docId,@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);
			String duality = null;
		...
		PublicSafetyRoomDAO.updatePublicSafetyRoom(map, parentId, id, null, null, geom, duality, partialBoundedBy );
	...
	public void deletePublicSafetyRoom(@PathVariable("docId") String docId,@PathVariable("id") String id, @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			Container container = applicationContext.getBean(Container.class);
			IndoorGMLMap map = container.getDocument(docId);			
			PublicSafetyRoomDAO.deletePublicSafetyRoom(map, id);
		}
		catch(NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
