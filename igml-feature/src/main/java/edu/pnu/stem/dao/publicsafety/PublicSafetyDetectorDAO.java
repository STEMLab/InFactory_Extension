...
import edu.pnu.stem.feature.publicsafety.PublicSafetyDetector;

public class PublicSafetyDetectorDAO {
	public static PublicSafetyDetector readPublicSafetyDetector(IndoorGMLMap map, String id) {
		PublicSafetyDetector target = null;
		try {
			target = (PublicSafetyDetector) map.getFeature(id);
	...
	public static PublicSafetyDetector createPublicSafetyDetector(IndoorGMLMap map, String parentId, String id, String name, String description,	Geometry geometry, String duality, List<String> connects) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyDetector newFeature = new PublicSafetyDetector(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyDetector) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyDetector", newFeature);
	...
	public static PublicSafetyDetector createPublicSafetyDetector(IndoorGMLMap map, String parentId, String id, String geometry,	List<String> connected) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}
	...
	public static void deletePublicSafetyDetector(IndoorGMLMap map, String id) {
		PublicSafetyDetector target = (PublicSafetyDetector) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (PublicSafetyDetector) map.getFutureFeature(id);
		}
	...
	public static PublicSafetyDetector updatePublicSafetyDetector(IndoorGMLMap map, String parentId, String id, String name, String description,
			Geometry geometry, String duality, List<String> connects) {
		
		PublicSafetyDetector result = new PublicSafetyDetector(map, id);
		PublicSafetyDetector target = (PublicSafetyDetector) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyDetector", result);
