...
import edu.pnu.stem.feature.publicsafety.PublicSafetyShutoff;

public class PublicSafetyShutoffDAO {

	public static PublicSafetyShutoff readPublicSafetyShutoff(IndoorGMLMap map, String id) {
		PublicSafetyShutoff target = null;
		try {
			target = (PublicSafetyShutoff) map.getFeature(id);
	...
	public static PublicSafetyShutoff createPublicSafetyShutoff(IndoorGMLMap map, String parentId, String id, String name, String description,	Geometry geometry, String duality, List<String> connects) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyShutoff newFeature = new PublicSafetyShutoff(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyShutoff) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyShutoff", newFeature);
	...
	public static PublicSafetyShutoff createPublicSafetyShutoff(IndoorGMLMap map, String parentId, String id, String geometry,	List<String> connected) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}
	...
	public static void deletePublicSafetyShutoff(IndoorGMLMap map, String id) {
		PublicSafetyShutoff target = (PublicSafetyShutoff) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (PublicSafetyShutoff) map.getFutureFeature(id);
		}
	...
	public static PublicSafetyShutoff updatePublicSafetyShutoff(IndoorGMLMap map, String parentId, String id, String name, String description,
			Geometry geometry, String duality, List<String> connects) {
		
		PublicSafetyShutoff result = new PublicSafetyShutoff(map, id);
		PublicSafetyShutoff target = (PublicSafetyShutoff) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyShutoff", result);
