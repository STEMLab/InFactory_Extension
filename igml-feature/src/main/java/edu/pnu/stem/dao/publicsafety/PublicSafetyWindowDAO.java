...
import edu.pnu.stem.feature.publicsafety.PublicSafetyWindow;
import edu.pnu.stem.geometry.jts.WKTReader3D;

public class PublicSafetyWindowDAO {
	
	public static PublicSafetyWindow createPublicSafetyWindow(IndoorGMLMap map, String parentId, String id, String geometry, String duality) {

		if (id == null) {
			id = UUID.randomUUID().toString();
		}
		PublicSafetyWindow newFeature = new PublicSafetyWindow(map, id);

		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyWindow) map.getFutureFeature(id);

		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyWindow", newFeature);
	...
	public static PublicSafetyWindow readPublicSafetyWindow(IndoorGMLMap map, String id) {
		PublicSafetyWindow target = null;
		try {
			target = (PublicSafetyWindow) map.getFeature(id);
	...
	public static PublicSafetyWindow createPublicSafetyWindow(IndoorGMLMap map, String parentId, String id, String name, String description, Geometry geometry, String duality, String fireEscape, String lockType, String openable, String material, float sizeHeight, float sizeWidth, String windowHanlding) {
		...
		if(windowHanlding != null) {
			newFeature.setWindowHandling(windowHanlding);
			System.out.println(windowHanlding);
		}

		return newFeature;
	}

	public static PublicSafetyWindow updatePublicSafetyWindow(IndoorGMLMap map, String parentId, String id, String name,
			String description, Geometry geometry, String duality) {
		PublicSafetyWindow result = new PublicSafetyWindow(map, id);
		PublicSafetyWindow target = (PublicSafetyWindow) map.getFeature(id);
		...
		map.setFeature(id, "PublicSafetyWindow", result);
		return result;

	}

	public static void deletePublicSafetyWindow(IndoorGMLMap map, String id) {
		PublicSafetyWindow target = (PublicSafetyWindow) map.getFeature(id);
		PrimalSpaceFeatures parent = target.getParent();
		parent.deleteCellSpaceBoundaryMember(id);
