...
import edu.pnu.stem.feature.publicsafety.PublicSafetyManual;

public class PublicSafetyManualDAO {

	public static PublicSafetyManual readPublicSafetyManual(IndoorGMLMap map, String id) {
		PublicSafetyManual target = null;
		try {
			target = (PublicSafetyManual) map.getFeature(id);
	...
	public static PublicSafetyManual createPublicSafetyManual(IndoorGMLMap map, String parentId, String id, String name, String description,	Geometry geometry, String duality, List<String> connects) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyManual newFeature = new PublicSafetyManual(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyManual) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyManual", newFeature);
	...
	public static PublicSafetyManual createPublicSafetyManual(IndoorGMLMap map, String parentId, String id, String geometry,	List<String> connected) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}
	...
	public static void deletePublicSafetyManual(IndoorGMLMap map, String id) {
		PublicSafetyManual target = (PublicSafetyManual) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (PublicSafetyManual) map.getFutureFeature(id);
		}
	...
	public static PublicSafetyManual updatePublicSafetyManual(IndoorGMLMap map, String parentId, String id, String name, String description,
			Geometry geometry, String duality, List<String> connects) {
		
		PublicSafetyManual result = new PublicSafetyManual(map, id);
		PublicSafetyManual target = (PublicSafetyManual) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyManual", result);
