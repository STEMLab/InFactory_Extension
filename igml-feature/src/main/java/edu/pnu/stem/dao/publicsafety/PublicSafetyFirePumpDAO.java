...
import edu.pnu.stem.feature.publicsafety.PublicSafetyFirePump;

public class PublicSafetyFirePumpDAO {

	public static PublicSafetyFirePump readPublicSafetyFirePump(IndoorGMLMap map, String id) {
		PublicSafetyFirePump target = null;
		try {
			target = (PublicSafetyFirePump) map.getFeature(id);
	...
	public static PublicSafetyFirePump createPublicSafetyFirePump(IndoorGMLMap map, String parentId, String id, String name, String description,	Geometry geometry, String duality, List<String> connects) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyFirePump newFeature = new PublicSafetyFirePump(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyFirePump) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyFirePump", newFeature);
	...
	public static PublicSafetyFirePump createPublicSafetyFirePump(IndoorGMLMap map, String parentId, String id, String geometry,	List<String> connected) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}
	...
	public static void deletePublicSafetyFirePump(IndoorGMLMap map, String id) {
		PublicSafetyFirePump target = (PublicSafetyFirePump) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (PublicSafetyFirePump) map.getFutureFeature(id);
		}
	...
	public static PublicSafetyFirePump updatePublicSafetyFirePump(IndoorGMLMap map, String parentId, String id, String name, String description,
			Geometry geometry, String duality, List<String> connects) {
		
		PublicSafetyFirePump result = new PublicSafetyFirePump(map, id);
		PublicSafetyFirePump target = (PublicSafetyFirePump) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyFirePump", result);
