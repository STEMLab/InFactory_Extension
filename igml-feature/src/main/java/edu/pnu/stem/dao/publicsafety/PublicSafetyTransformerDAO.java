...
import edu.pnu.stem.feature.publicsafety.PublicSafetyTransformer;

public class PublicSafetyTransformerDAO {

	public static PublicSafetyTransformer readPublicSafetyTransformer(IndoorGMLMap map, String id) {
		PublicSafetyTransformer target = null;
		try {
			target = (PublicSafetyTransformer) map.getFeature(id);
	...
	public static PublicSafetyTransformer createPublicSafetyTransformer(IndoorGMLMap map, String parentId, String id, String name, String description,	Geometry geometry, String duality, List<String> connects) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyTransformer newFeature = new PublicSafetyTransformer(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyTransformer) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyTransformer", newFeature);
	...
	public static PublicSafetyTransformer createPublicSafetyTransformer(IndoorGMLMap map, String parentId, String id, String geometry,	List<String> connected) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}
	...
	public static void deletePublicSafetyTransformer(IndoorGMLMap map, String id) {
		PublicSafetyTransformer target = (PublicSafetyTransformer) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (PublicSafetyTransformer) map.getFutureFeature(id);
		}
	...
	public static PublicSafetyTransformer updatePublicSafetyTransformer(IndoorGMLMap map, String parentId, String id, String name, String description,
			Geometry geometry, String duality, List<String> connects) {
		
		PublicSafetyTransformer result = new PublicSafetyTransformer(map, id);
		PublicSafetyTransformer target = (PublicSafetyTransformer) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyTransformer", result);
