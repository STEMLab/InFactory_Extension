...
public class PublicSafetyKeyBoxDAO {

	public static PublicSafetyKeyBox readPublicSafetyKeyBox(IndoorGMLMap map, String id) {
		PublicSafetyKeyBox target = null;
		try {
			target = (PublicSafetyKeyBox) map.getFeature(id);
	...
	public static PublicSafetyKeyBox createPublicSafetyKeyBox(IndoorGMLMap map, String parentId, String id, String name, String description,	Geometry geometry, String duality, List<String> connects, String PSLegend) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyKeyBox newFeature = new PublicSafetyKeyBox(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyKeyBox) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyKeyBox", newFeature);
	...
	public static PublicSafetyKeyBox createPublicSafetyKeyBox(IndoorGMLMap map, String parentId, String id, String geometry,	List<String> connected) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}
	...
	public static void deletePublicSafetyKeyBox(IndoorGMLMap map, String id) {
		PublicSafetyKeyBox target = (PublicSafetyKeyBox) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (PublicSafetyKeyBox) map.getFutureFeature(id);
		}
	...
	public static PublicSafetyKeyBox updatePublicSafetyKeyBox(IndoorGMLMap map, String parentId, String id, String name, String description,
			Geometry geometry, String duality, List<String> connects) {
		
		PublicSafetyKeyBox result = new PublicSafetyKeyBox(map, id);
		PublicSafetyKeyBox target = (PublicSafetyKeyBox) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyKeyBox", result);
