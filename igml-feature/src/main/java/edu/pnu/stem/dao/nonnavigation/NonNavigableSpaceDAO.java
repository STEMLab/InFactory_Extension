package edu.pnu.stem.dao.nonnavigation;
...
public class NonNavigableSpaceDAO {
	public static void deleteNonNavigableSpace(IndoorGMLMap map, String id) {
		NonNavigableSpace target = (NonNavigableSpace) map.getFeature(id);
	...
	public static NonNavigableSpace readNonNavigableSpace(IndoorGMLMap map, String id) {
		NonNavigableSpace target = null;
		try {
			target = (NonNavigableSpace) map.getFeature(id);
	...
	public static NonNavigableSpace updateNonNavigableSpace(IndoorGMLMap map, String parentId, String id, String name,
			String description, Geometry geometry, String duality, List<String> partialboundedBy) {
		NonNavigableSpace result = new NonNavigableSpace(map, id);
		NonNavigableSpace target = (NonNavigableSpace) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "NonNavigableSpace", result);

		return result;
	}

	public static NonNavigableSpace createNonNavigableSpace(IndoorGMLMap map, String parentId, String id, String name, String description, Geometry geometry, String duality, List<String> partialBoundedBy, String obstacleType) {
		System.out.println("NonNavigableSpaceDAO");
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		NonNavigableSpace newFeature = new NonNavigableSpace(map, id);

		if (map.hasFutureID(id)) {
			newFeature = (NonNavigableSpace) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "NonNavigableSpace", newFeature);
        ...
        if(obstacleType != null) {
			...
			newFeature.setObstacleType(obstacleType);
	...
	public static NonNavigableSpace createNonNavigableSpace(IndoorGMLMap map, String parentId, String id, String geometry,
			String duality) {
