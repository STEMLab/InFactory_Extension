...
import edu.pnu.stem.feature.publicsafety.PublicSafetyAlarm;

public class PublicSafetyAlarmDAO {
	public static PublicSafetyAlarm readPublicSafetyAlarm(IndoorGMLMap map, String id) {
		PublicSafetyAlarm target = null;
		try {
			target = (PublicSafetyAlarm) map.getFeature(id);
	...
	public static PublicSafetyAlarm createPublicSafetyAlarm(IndoorGMLMap map, String parentId, String id, String name, String description,	Geometry geometry, String duality, List<String> connects) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyAlarm newFeature = new PublicSafetyAlarm(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyAlarm) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyAlarm", newFeature);
	...
	public static PublicSafetyAlarm createPublicSafetyAlarm(IndoorGMLMap map, String parentId, String id, String geometry,	List<String> connected) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}
	...
	public static void deletePublicSafetyAlarm(IndoorGMLMap map, String id) {
		PublicSafetyAlarm target = (PublicSafetyAlarm) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (PublicSafetyAlarm) map.getFutureFeature(id);
		}
	...
	public static PublicSafetyAlarm updatePublicSafetyAlarm(IndoorGMLMap map, String parentId, String id, String name, String description,
			Geometry geometry, String duality, List<String> connects) {
		
		PublicSafetyAlarm result = new PublicSafetyAlarm(map, id);
		PublicSafetyAlarm target = (PublicSafetyAlarm) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyAlarm", result);
