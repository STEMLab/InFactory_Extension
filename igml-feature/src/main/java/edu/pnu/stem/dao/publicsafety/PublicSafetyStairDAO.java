...
public class PublicSafetyStairDAO {
	public static void deletePublicSafetyStair(IndoorGMLMap map, String id) {
		PublicSafetyStair target = (PublicSafetyStair) map.getFeature(id);
	...
	public static PublicSafetyStair readPublicSafetyStair(IndoorGMLMap map, String id) {
		PublicSafetyStair target = null;
		try {
			target = (PublicSafetyStair) map.getFeature(id);
	...
	public static PublicSafetyStair updatePublicSafetyStair(IndoorGMLMap map, String parentId, String id,
			String name, String description, Geometry geometry, String duality, List<String> partialboundedBy) {
		PublicSafetyStair result = new PublicSafetyStair(map, id);
		PublicSafetyStair target = (PublicSafetyStair) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyStair", result);

		return result;
	}

	public static PublicSafetyStair createPublicSafetyStair(IndoorGMLMap map, String parentId, String id,
			String name, String description, Geometry geometry, String duality, List<String> partialBoundedBy,
			String classType, String functionType, String usageType) {
		System.out.println("PublicSafetyStairDAO");
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyStair newFeature = new PublicSafetyStair(map, id);

		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyStair) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyStair", newFeature);
