...
import edu.pnu.stem.feature.publicsafety.PublicSafetyGenerator;

public class PublicSafetyGeneratorDAO {

	public static PublicSafetyGenerator readPublicSafetyGenerator(IndoorGMLMap map, String id) {
		PublicSafetyGenerator target = null;
		try {
			target = (PublicSafetyGenerator) map.getFeature(id);
	...
	public static PublicSafetyGenerator createPublicSafetyGenerator(IndoorGMLMap map, String parentId, String id, String name, String description,	Geometry geometry, String duality, List<String> connects) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyGenerator newFeature = new PublicSafetyGenerator(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyGenerator) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyGenerator", newFeature);
	...
	public static PublicSafetyGenerator createPublicSafetyGenerator(IndoorGMLMap map, String parentId, String id, String geometry,	List<String> connected) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}
	...
	public static void deletePublicSafetyGenerator(IndoorGMLMap map, String id) {
		PublicSafetyGenerator target = (PublicSafetyGenerator) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (PublicSafetyGenerator) map.getFutureFeature(id);
		}
	...
	public static PublicSafetyGenerator updatePublicSafetyGenerator(IndoorGMLMap map, String parentId, String id, String name, String description,
			Geometry geometry, String duality, List<String> connects) {
		
		PublicSafetyGenerator result = new PublicSafetyGenerator(map, id);
		PublicSafetyGenerator target = (PublicSafetyGenerator) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyGenerator", result);
