...
import edu.pnu.stem.feature.publicsafety.PublicSafetyMedical;

public class PublicSafetyMedicalDAO {

	public static PublicSafetyMedical readPublicSafetyMedical(IndoorGMLMap map, String id) {
		PublicSafetyMedical target = null;
		try {
			target = (PublicSafetyMedical) map.getFeature(id);
	...
	public static PublicSafetyMedical createPublicSafetyMedical(IndoorGMLMap map, String parentId, String id, String name, String description,	Geometry geometry, String duality, List<String> connects) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}

		PublicSafetyMedical newFeature = new PublicSafetyMedical(map, id);
		if (map.hasFutureID(id)) {
			newFeature = (PublicSafetyMedical) map.getFutureFeature(id);
			// map.removeFutureID(id);
		} else {
			map.setFutureFeature(id, newFeature);
		}
		map.setFeature(id, "PublicSafetyMedical", newFeature);
	...
	public static PublicSafetyMedical createPublicSafetyMedical(IndoorGMLMap map, String parentId, String id, String geometry,	List<String> connected) {
		if (id == null) {
			id = UUID.randomUUID().toString();
		}
	...
	public static void deletePublicSafetyMedical(IndoorGMLMap map, String id) {
		PublicSafetyMedical target = (PublicSafetyMedical) map.getFeature(id);

		if (target == null) {
			if (map.hasFutureID(id))
				target = (PublicSafetyMedical) map.getFutureFeature(id);
		}
	...
	public static PublicSafetyMedical updatePublicSafetyMedical(IndoorGMLMap map, String parentId, String id, String name, String description,
			Geometry geometry, String duality, List<String> connects) {
		
		PublicSafetyMedical result = new PublicSafetyMedical(map, id);
		PublicSafetyMedical target = (PublicSafetyMedical) map.getFeature(id);
		...
		map.removeFeature(id);
		map.setFeature(id, "PublicSafetyMedical", result);
